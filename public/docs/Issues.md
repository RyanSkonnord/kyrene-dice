...
* A completely different problem in WoD mechanics that has also captured my interest: given a character sheet for an advanced character, find the optimum way to do the initial character creation, in order to minimize the number of experience points needed to raise the stats to match the final character sheet.
    * It doesn't have any particular relation to the dice pool calculations other than coming from the same game line. But such a solver would want some kind of character sheet UI, and it would probably make sense to fold it into this webapp.
    * I want to emphasize, this kind of solver would be meant only for advanced-start characters (e.g., spend 75 XP at character creation to start as an ancilla vampire instead of a neonate), to calculate the equivalent XP value of the player's desired stat array. It is not intended to help plan a future build, and I don't recommend trying to "optimize" the character creation stage in order to penny-pinch later XP costs.
