...
A guiding principle of my implementation is that I wanted the code representing the game rules to govern both the die roller and the probability calculator equally. Write it one place, evaluate it in two: to apply the rules both to an actual roll with particular random numbers, and to the general distribution of all possible rolls. This requires a nontrivial amount of abstraction, but once I laid that groundwork, it was very satisfying to see full probability tables springing to life that can account for how a particular roleplaying group handles 1s.

Similar Tools
-------------

As I begin this project, the state of the art for this particular domain niche seems to be the [World of Darkness Dice Pool Calculator on tablegameshub.com](https://tablegameshub.com/world-of-darkness-dice-pool-calculator/). Kyrene Dice tries to improve on that tool by tying the calculator interface a bit more closely to actual game rules and a usable die-roller.
